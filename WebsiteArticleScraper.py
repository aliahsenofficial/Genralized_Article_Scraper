...
def scrape_html(html_source):
    ...
    return article_title, article_date, article_body.replace('\n', '').replace('\t', '').replace('  ', '')
...
html_source = scrape_html(response.text)
# print(scrape_html(response.text)[0])
with open("article.txt", "w") as text_file:
    #write string to file
    text_file.write('Title: ' + html_source[0] + "\n")
    text_file.write('Date Posted: ' + html_source[1] + "\n")
    text_file.write(html_source[2] + "\n")